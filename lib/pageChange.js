...
pageChange._pageEnd = function () {
  transport.timing(pageLoadedId, (new Date()) - pageLoadedAt)
}
pageChange._logPage = function () {
  var navStart = new Date()
  barometer.pageStartedAt = navStart
  transport.count(pageLoadStats._createCounterName('visits'))
  if (pageLoadedAt) {
    pageChange._pageEnd()
  }
  pageCounter++
  ...
  var stop = false
  var delay = 25
  var metric = pageLoadStats._createTimingName('dynamic')
  var done = function () {
    clearTimeout(timeout)
    pageHasLoaded = true
    pageLoadedAt = new Date()
    pageLoadedId = pageLoadStats._createTimingName('engagement')
    var overhead = 25 * 5
    transport.timing(metric, ((new Date()) - navStart) - overhead)
